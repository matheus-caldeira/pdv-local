...
export function PDV() {
  ...
  const [payOption, setPayOption] = useState<'now' | 'tab' | 'delivery' | null>(
    null,
  );
  ...
  const total = cart.reduce(
    (s, i) => s + (i.salePrice + (i.customizationTotal || 0)) * i.qty,
    0,
  );
  ...
  function addSimpleToCart(product: Product) {
    setCart((prev) => {
      const existing = prev.find(
        (i) => i.productId === product.id! && !i.customizations?.length,
      );
      if (existing) {
        return prev.map((i) =>
  ...
  function calcCustomizationTotal(modal: CustomModalState): number {
    ...
    for (const group of modal.groups) {
      ...
      const selectedEntries = Object.entries(groupSelections).filter(
        ([, qty]) => qty > 0,
      );
    ...
    for (const group of groups) {
      if (group.required) {
        ...
        if (totalSelected < (group.minQty || 1)) {
          toast(
            `Selecione pelo menos ${group.minQty || 1} em "${group.name}"`,
            'error',
          );
          return;
        }
  ...
  function saveObs() {
    ...
    setCart((prev) =>
      prev.map((i) =>
        i.cartId === editingObsId
          ? { ...i, observation: obsText.trim() || undefined }
          : i,
      ),
    );
  ...
  function statusForOption(
    option: 'now' | 'tab' | 'delivery',
  ): Order['status'] {
  ...
  return (
    <div className="pdv">
          ...
          {filteredProducts.length === 0 ? (
            ...
            filteredProducts.map((p) => {
              ...
              return (
                <button
                  key={p.id}
                  className={`cardapio-item ${inCartQty > 0 ? 'in-cart' : ''}`}
                  onClick={() => handleProductClick(p)}
                >
                  <span className="cardapio-name">{p.name}</span>
                  <span className="cardapio-price">
                    {formatMoney(p.salePrice)}
                  </span>
                  {hasCustom && (
                    <span className="cardapio-custom-badge">+</span>
                  )}
                  {inCartQty > 0 && (
                    <span className="cardapio-qty">{inCartQty}</span>
                  )}
                </button>
              );
            })
              ...
              <div key={item.cartId} className="cart-item">
                <div className="cart-item-info">
                  ...
                  <span className="cart-item-price">
                    {formatMoney(itemUnitTotal(item))} un.
                  </span>
                </div>
                {item.customizations &&
                  item.customizations.map((c, i) => (
                    <div key={i} className="cart-item-customs">
                      {c.items.map((ci, j) => (
                        <span key={j} className="cart-custom-tag">
                          {ci.qty > 1 ? ci.qty + 'x ' : ''}
                          {ci.name}
                        </span>
                      ))}
                ...
                {item.observation && (
                  <div className="cart-item-obs">Obs: {item.observation}</div>
                )}
                <div className="cart-item-controls">
                  <span className="cart-item-subtotal tabular">
                    {formatMoney(itemUnitTotal(item) * item.qty)}
                  </span>
                  <button
                    className="qty-btn qty-btn-obs"
        ...
        {cart.length > 0 && (
          <div className="cart-footer">
            <div className="cart-total">
              <span>Total</span>
              <span className="cart-total-value tabular">
                {formatMoney(total)}
              </span>
            </div>
            <button
        ...
        <div className={`pdv-summary-bar ${summaryExpanded ? 'expanded' : ''}`}>
                  ...
                  {customerSuggestions.length > 0 && (
                    <div className="customer-suggestions">
                      {customerSuggestions.map((c) => (
                        ...
                        >
                          <span>{c.name}</span>
                          <span className="customer-suggestion-phone">
                            {c.phone}
                          </span>
                        </button>
                      ))}
              ...
              {cart.map((item) => (
                <div key={item.cartId} className="summary-item">
                  <div className="summary-item-left">
                    <span className="summary-item-qty tabular">
                      {item.qty}x
                    </span>
                    <div>
                      ...
                      {item.customizations && (
                        <span className="summary-item-customs">
                          {item.customizations
                            .flatMap((c) => c.items.map((ci) => ci.name))
                            .join(', ')}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="summary-item-right">
                    <span className="summary-item-total tabular">
                      {formatMoney(itemUnitTotal(item) * item.qty)}
                    </span>
                    <button
                      className="qty-btn-sm qty-btn-obs"
                      onClick={() => openObsEdit(item.cartId)}
                    >
                    ...
                    <button
                      className="qty-btn-sm qty-btn-remove"
                      onClick={() => removeCartItem(item.cartId)}
                    >
                      <Trash2 size={12} />
                    </button>
          ...
          <div className="summary-footer">
            ...
            >
              ...
              <span className="summary-count tabular">
      ...
      <Modal
        ...
        title={customModal?.product.name || ''}
      >
        {customModal && (
          <div className="custom-modal">
            <div className="custom-modal-price tabular">
              {formatMoney(customModal.product.salePrice)}
            </div>
              ...
              return (
                <div key={group.id} className="custom-modal-group">
                  <div className="custom-modal-group-header">
                    <div>
                      <span className="custom-modal-group-name">
                        {group.name}
                      </span>
                      <span className="custom-modal-group-range">
                        ...
                        {group.chargeAfter > 0 &&
                          ` · ${group.chargeAfter} gratis`}
                      </span>
                    </div>
                    <span className="custom-modal-group-count tabular">
                    ...
                    return (
                      <div key={item.id} className="custom-modal-item">
                        <div className="custom-modal-item-info">
                          <span className="custom-modal-item-name">
                            {item.name}
                          </span>
                          {item.price > 0 && (
                            <span className="custom-modal-item-price tabular">
                              ...
                              {item.chargeAfter > 0 && (
                                <span className="custom-modal-item-free">
                                  {' '}
                          ...
                          {(item.maxQty || 0) > 0 && (
                            <span className="custom-modal-item-max">
                              max {item.maxQty}
                            </span>
                          )}
                        </div>
                        <div className="custom-modal-item-controls">
                          {qty > 0 && (
                            <>
                              <button
                                className="qty-btn-sm"
                                onClick={() =>
                                  updateCustomSelection(group.id!, item.id!, -1)
                                }
                              >
                                <Minus size={12} />
                              </button>
                          ...
                          <button
                            className="qty-btn-sm qty-btn-add"
                            onClick={() =>
                              updateCustomSelection(group.id!, item.id!, 1)
                            }
                            disabled={
            ...
            >
              Adicionar ·{' '}
              {formatMoney(
                customModal.product.salePrice +
                  calcCustomizationTotal(customModal),
              )}
            </button>
          </div>
        )}
      ...
      <Modal
        ...
        title={
          paymentStep === 'option' ? 'Como deseja pagar?' : 'Forma de Pagamento'
        }
      >
        ...
        {paymentStep === 'option' ? (
          ...
          <>
            <div className="payment-methods">
              {PAYMENT_METHODS.filter((pm) => pm.key !== 'pagar_depois').map(
                (pm) => (
                  <button
                    key={pm.key}
