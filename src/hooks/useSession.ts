...
export function useSession() {
  ...
  const refresh = useCallback(async () => {
    ...
    if (session) {
      setActiveSession(session.closedAt === null || session.closedAt === 0 ? session : null)
    } else {
  ...
  const openSession = useCallback(async (cashInitial: number) => {
  ...
  const closeSession = useCallback(async (cashFinal: number, notes: string = '') => {
