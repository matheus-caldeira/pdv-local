...
import { db, type CustomizationGroup, type CustomizationItem } from '../db/database'
...
export function Customizations() {
  ...
  function openNewGroup() {
    setEditingGroup({ name: '', required: false, minQty: 0, maxQty: 10, chargeAfter: 0 })
    setGroupModal(true)
  }
    ...
    for (const p of products) {
      if (p.customizationGroupIds?.includes(id)) {
        await db.products.update(p.id!, {
          customizationGroupIds: p.customizationGroupIds.filter(gid => gid !== id)
        })
      }
    }
  ...
  return (
    <div className="custom-page">
      ...
      <p className="custom-desc">
        Crie grupos de customizacao (ex: Adicionais, Tipo do pao) e adicione itens com precos.
        Vincule os grupos aos produtos na tela de Produtos.
      </p>
          ...
          {groups.map(g => {
            ...
            return (
              <div key={g.id} className="custom-group-card">
                <div className="custom-group-header" onClick={() => setExpandedGroup(isExpanded ? null : g.id!)}>
                  ...
                  <div className="custom-group-info">
                    ...
                    <div className="custom-group-meta">
                      {g.required && <span className="custom-tag required">Obrigatorio</span>}
                      {!g.required && <span className="custom-tag">Opcional</span>}
                      <span className="custom-tag">Min: {g.minQty} / Max: {g.maxQty}</span>
                      {g.chargeAfter > 0 && <span className="custom-tag">Cobra a partir de {g.chargeAfter}</span>}
                      <span className="custom-tag">{groupItems.length} {groupItems.length === 1 ? 'item' : 'itens'}</span>
                    </div>
                  </div>
                  <div className="custom-group-actions" onClick={e => e.stopPropagation()}>
                    <button className="btn btn-ghost" onClick={() => openEditGroup(g)}><Pencil size={14} /></button>
                    <button className="btn btn-ghost" style={{ color: 'var(--danger)' }} onClick={() => removeGroup(g.id!)}><Trash2 size={14} /></button>
                  </div>
                </div>
                ...
                {isExpanded && (
                  <div className="custom-group-body">
                    {groupItems.length === 0 ? (
                      <div className="custom-items-empty">Nenhum item neste grupo</div>
                    ) : (
                      groupItems.map(item => (
                        <div key={item.id} className={`custom-item-row ${!item.active ? 'inactive' : ''}`}>
                          ...
                          <div className="custom-item-actions">
                            <button className="btn btn-ghost" onClick={() => openEditItem(item)}><Pencil size={14} /></button>
                            <button className="btn btn-ghost" style={{ color: 'var(--danger)' }} onClick={() => removeItem(item.id!)}><Trash2 size={14} /></button>
                          </div>
                        </div>
                      ))
                    )}
                    <button className="btn btn-outline btn-sm custom-add-item" onClick={() => openNewItem(g.id!)}>
      ...
      {/* Group modal */}
      <Modal open={groupModal} onClose={() => setGroupModal(false)} title={editingGroup.id ? 'Editar Grupo' : 'Novo Grupo'}>
        <div className="form-grid" style={{ gridTemplateColumns: '1fr' }}>
          ...
          <div className="form-field">
            ...
            <select value={editingGroup.required ? '1' : '0'}
              onChange={e => setEditingGroup(g => ({ ...g, required: e.target.value === '1' }))}>
        ...
        <button className="btn btn-accent btn-full" onClick={saveGroup} style={{ marginTop: 'var(--space-4)' }}>Salvar</button>
      </Modal>

      {/* Item modal */}
      <Modal open={itemModal} onClose={() => setItemModal(false)} title={editingItem.id ? 'Editar Item' : 'Novo Item'}>
        <div className="form-grid" style={{ gridTemplateColumns: '1fr' }}>
          <div className="form-field">
            <label>Nome</label>
          ...
          <div className="form-field">
            <label>Status</label>
            <select value={editingItem.active !== false ? '1' : '0'}
              onChange={e => setEditingItem(i => ({ ...i, active: e.target.value === '1' }))}>
              <option value="1">Ativo</option>
              <option value="0">Inativo</option>
            </select>
          </div>
        </div>
        <button className="btn btn-accent btn-full" onClick={saveItem} style={{ marginTop: 'var(--space-4)' }}>Salvar</button>
      </Modal>
    </div>
  )
}
