...
export function Cash() {
  ...
  async function loadData() {
    ...
    if (activeSession?.id) {
      const mvts = await db.cashMovements.where('sessionId').equals(activeSession.id).toArray()
      setMovements(mvts)

      const orders = await db.orders.where('sessionId').equals(activeSession.id).toArray()
  ...
  async function addMovement() {
    ...
    toast(movementModal === 'sangria' ? 'Sangria registrada' : 'Suprimento registrado')
    loadData()
  }
  ...
  const expectedCash = (activeSession?.cashInitial || 0) + totalDinheiro + totalSuprimentos - totalSangrias
    ...
    <div className="cash-page">
      ...
      {!activeSession ? (
        ...
        <div className="cash-active">
          <div className="cash-status-bar">
            ...
            <button className="btn btn-outline btn-sm" onClick={() => setCloseModal(true)}>
          ...
          <div className="cash-summary-grid">
            <div className="cash-summary-card">
              ...
              <span className="cash-sum-value tabular">{formatMoney(activeSession.cashInitial)}</span>
            </div>
            <div className="cash-summary-card">
              <span className="cash-sum-label">Vendas Dinheiro</span>
              <span className="cash-sum-value tabular" style={{ color: 'var(--success)' }}>+ {formatMoney(totalDinheiro)}</span>
            </div>
            <div className="cash-summary-card">
              <span className="cash-sum-label">Suprimentos</span>
              <span className="cash-sum-value tabular" style={{ color: 'var(--info)' }}>+ {formatMoney(totalSuprimentos)}</span>
            </div>
            <div className="cash-summary-card">
              <span className="cash-sum-label">Sangrias</span>
              <span className="cash-sum-value tabular" style={{ color: 'var(--danger)' }}>- {formatMoney(totalSangrias)}</span>
            </div>
            <div className="cash-summary-card cash-sum-total">
              <span className="cash-sum-label">Esperado no Caixa</span>
              <span className="cash-sum-value-lg tabular">{formatMoney(expectedCash)}</span>
            </div>
          </div>
              ...
              <div className="cash-methods-list">
                {Object.entries(summary).map(([method, total]) => (
                  <div key={method} className="cash-method-row">
                    ...
                    <span className="tabular" style={{ fontWeight: 'var(--weight-bold)' }}>{formatMoney(total)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="cash-movement-actions">
            <button className="btn btn-outline" onClick={() => setMovementModal('sangria')}>
              <ArrowUpCircle size={18} /> Sangria
            </button>
            <button className="btn btn-outline" onClick={() => setMovementModal('suprimento')}>
          ...
          {movements.length > 0 && (
            <div className="cash-movements">
              ...
              {movements.sort((a, b) => b.createdAt - a.createdAt).map(m => (
                <div key={m.id} className="movement-row">
                  <div>
                    <span className={`movement-type ${m.type}`}>
                      {m.type === 'sangria' ? 'Sangria' : 'Suprimento'}
                    </span>
                    {m.reason && <span className="movement-reason">{m.reason}</span>}
                  </div>
                  <div className="movement-right">
                    <span className={`movement-amount tabular ${m.type}`}>
                      {m.type === 'sangria' ? '- ' : '+ '}{formatMoney(m.amount)}
                    </span>
                    <span className="movement-time">{formatTime(m.createdAt)}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      {sessions.filter(s => s.closedAt).length > 0 && (
        <div className="past-sessions">
          ...
          {sessions.filter(s => s.closedAt).map(s => (
            <div key={s.id} className="past-session-row">
              <div>
                <span className="past-date">{formatDate(s.openedAt)}</span>
                <span className="past-time">{formatTime(s.openedAt)} - {formatTime(s.closedAt!)}</span>
              </div>
              <div className="past-cash">
                <span className="past-initial">Inicial: {formatMoney(s.cashInitial)}</span>
                <span className="past-final">Final: {formatMoney(s.cashFinal || 0)}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      >
        ...
        <button className="btn btn-accent btn-full" onClick={addMovement} style={{ marginTop: 'var(--space-4)' }}>
          Confirmar
        </button>
      </Modal>
      ...
      <Modal open={closeModal} onClose={() => setCloseModal(false)} title="Fechar Caixa">
        <p style={{ fontSize: 'var(--text-sm)', color: 'var(--ink-secondary)', marginBottom: 'var(--space-4)' }}>
          Valor esperado: <strong className="tabular">{formatMoney(expectedCash)}</strong>
        </p>
        <div className="form-grid" style={{ gridTemplateColumns: '1fr' }}>
          <div className="form-field">
        ...
        <button className="btn btn-accent btn-full" onClick={handleClose} style={{ marginTop: 'var(--space-4)' }}>
