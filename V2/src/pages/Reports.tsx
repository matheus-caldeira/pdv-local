...
export function Reports() {
  ...
  useEffect(() => {
    ...
    db.orders.where('sessionId').equals(selectedSession).toArray().then(setOrders)
  }, [selectedSession])

  const paidOrders = orders.filter(o => o.status === 'paid')
  const totalSales = paidOrders.reduce((s, o) => s + o.total, 0)
  const totalCost = paidOrders.reduce((s, o) =>
    s + o.items.reduce((c, i) => c + i.costPrice * i.qty, 0), 0)
  ...
  paidOrders.forEach(o => {
    o.items.forEach(item => {
      const existing = byProduct.get(item.name) || { qty: 0, total: 0, cost: 0 }
        ...
        <>
          ...
          {currentSession && (
            <div className="report-period">
              {formatDate(currentSession.openedAt)} {formatTime(currentSession.openedAt)}
              {currentSession.closedAt && ` — ${formatTime(currentSession.closedAt)}`}
            </div>
          )}
          ...
          <div className="report-summary-grid">
            <div className="report-card report-card-main">
              <span className="report-card-value tabular">{formatMoney(totalSales)}</span>
              <span className="report-card-label">Total Vendas</span>
            </div>
            <div className="report-card">
              <span className="report-card-value tabular">{paidOrders.length}</span>
              <span className="report-card-label">Pedidos Pagos</span>
            </div>
            <div className="report-card">
              <span className="report-card-value tabular">{formatMoney(profit)}</span>
          ...
          <div className="report-section">
            ...
            <div className="report-table">
              {Object.entries(byMethod).map(([method, total]) => {
                const pct = totalSales > 0 ? ((total / totalSales) * 100).toFixed(0) : '0'
                return (
                  <div key={method} className="report-table-row">
                    <span className="report-table-name">{PAYMENT_LABELS[method] || method}</span>
                    <div className="report-bar-container">
                      <div className="report-bar" style={{ width: `${pct}%` }} />
                    </div>
                    <span className="report-table-pct">{pct}%</span>
                    <span className="report-table-value tabular">{formatMoney(total)}</span>
                  </div>
                )
              })}
            </div>
            ...
            <div className="report-table">
              {productRanking.map((p, i) => (
                <div key={p.name} className="report-table-row">
                  ...
                  <span className="report-table-value tabular">{formatMoney(p.total)}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {orders.filter(o => o.status === 'pending' || o.status === 'open').length > 0 && (
            <div className="report-section">
              ...
              <div className="report-table">
                {orders.filter(o => o.status === 'pending' || o.status === 'open').map(o => (
                  <div key={o.id} className="report-table-row">
                    <span className="report-table-name">#{o.ticket} {o.customerName}</span>
                    <span className={`status-badge status-${o.status}`}>
                      {o.status === 'open' ? 'Aberto' : 'Pendente'}
                    </span>
                    <span className="report-table-value tabular">{formatMoney(o.total)}</span>
                  </div>
                ))}
              </div>
            </div>
