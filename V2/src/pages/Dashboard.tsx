...
export function Dashboard() {
  ...
  const [stats, setStats] = useState<Stats>({
    totalSales: 0, orderCount: 0, openOrders: 0, topProducts: [], recentOrders: [],
  })
  ...
  async function loadStats(sessionId: number) {
    const orders = await db.orders.where('sessionId').equals(sessionId).toArray()
    ...
    <div className="dashboard">
      ...
      <div className="stats-grid">
        <div className="stat-card stat-highlight">
          ...
          <div className="stat-value tabular">{formatMoney(stats.totalSales)}</div>
            ...
            <div className="top-products">
              {stats.topProducts.map((p, i) => (
                <div key={p.name} className="top-product-row">
                  ...
                  <span className="top-total tabular">{formatMoney(p.total)}</span>
                </div>
              ))}
            </div>
          )}
              ...
              {stats.recentOrders.map(o => (
                <div key={o.id} className="recent-order-row">
                  <div>
                    ...
                    {o.customerName && <span className="recent-customer">{o.customerName}</span>}
                  </div>
                  <div className="recent-right">
                    <span className={`status-badge status-${o.status}`}>{statusLabel(o.status)}</span>
                    <span className="recent-total tabular">{formatMoney(o.total)}</span>
                  </div>
                </div>
              ))}
            </div>
